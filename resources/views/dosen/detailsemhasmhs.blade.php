...
<div class="container bg-light shadow mt-5 py-5 px-5">
    <table class="table table-sm table-borderless fs-5">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>:&nbsp;
                    @foreach ($dospems as $maha => $m)
                    @if (isset($m->berkas_semhas))
                    <a href="{{ asset($m->berkas_semhas) }}" class="btn btn-primary" download="">Download</a>
                    @else
                    Belum ada berkas
                    @endif
                    @endforeach
            ...
            <tr>
                ...
                <td>:&nbsp;
                    @foreach ($dospems as $maha => $m)
                    <a href="jadwalsemhas/{{ $m->id }}" class="btn btn-primary">Jadwal</a>
                    @endforeach
                </td>
            </tr>
        </tbody>
    ...
    @foreach ($dospems as $maha => $m)
    action="{{$m->id}}/updatestatussemhas" 
    @endforeach
