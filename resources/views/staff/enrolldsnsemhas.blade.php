...
<!-- Sale & Revenue Start -->
<form class="form-horizontal" action="/staff/jadwalsemhas/fix" method="post" enctype="multipart/form-data">
{{ csrf_field() }}
            ...
            <tr>
                ...
                <td>:&nbsp;&nbsp;
                    @foreach ($seminars as $sem => $s)
                        {{$s['judul']}}
                        <input type="hidden" name="id_prasemhas" value="{{$s['id']}}" required>
                    @endforeach
                </td>
            </tr>
            <tr>
