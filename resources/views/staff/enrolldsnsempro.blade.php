...
<div class="container mt-5">
    <h2 class="fs-1">Tambah Dosen Pembanding</h2>
</div>

<!-- Sale & Revenue Start -->
<form class="form-horizontal" action="/staff/jadwalsempro/fix" method="post" enctype="multipart/form-data">
{{ csrf_field() }}
            ...
            <tr>
                ...
                <td>:&nbsp;&nbsp;
                    @foreach ($seminars as $sem => $s)
                        {{$s['judul']}}
                        <input type="hidden" name="id_prasempro" value="{{$s['id']}}" required>
                    @endforeach
                </td>
            </tr>
            <tr>
